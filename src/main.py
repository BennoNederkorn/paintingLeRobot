...
def main():
    ...
    while(not robot_controller.has_robot_finished_image()): 
        new_speach_prompt : list[float] = speech_prorcessing.execute_recording()
        if new_speach_prompt == []:
            # continue
            pass

        new_text_prompt : str = speech_prorcessing.speech_to_text(new_speach_prompt)
